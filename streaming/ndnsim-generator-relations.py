...
class ConfGenerator:
	...
	def __writeConsumers(self):
		...
		for ic in sorted(self.nodes.consumers):
			...
			self.arqout.write(str.format('consumerHelper{}.SetAttribute ("HasSepAudio", BooleanValue (true));\n', clc))
			self.arqout.write(str.format('consumerHelper{}.SetAttribute ("SizeVideo", UintegerValue ({}));\n', clc, self.P['videochunkmax']))
			self.arqout.write(str.format('consumerHelper{}.SetAttribute ("SizeAudio", UintegerValue ({}));\n', clc, self.P['audiochunkmax']))
