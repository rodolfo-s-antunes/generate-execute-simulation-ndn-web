...
class ConfGenerator:
	...
	def __writeVersionControl(self):
		self.arqout.write('Ptr<WebVersionControl> wbc = Create<WebVersionControl>(false);\n')
		self.arqout.write('wbc->readSizeTrace("scratch/{}");\n'.format(self.P['sizetracefile']))
		self.arqout.write('wbc->insertAll({}, {}, 0.0, 0.0, {}, {}, {});\n'.format(self.P['catalogsize'], self.P['dataupfreq'], self.P['datasize'], self.P['layoutsize'], self.P['codesize']))
		return
	...
	def __writeConsumers(self):
		...
		for ic in sorted(self.nodes.consumers):
			...
			self.arqout.write('consumerHelper{}.SetAttribute("UsingRelations", BooleanValue(false));\n'.format(clc))
