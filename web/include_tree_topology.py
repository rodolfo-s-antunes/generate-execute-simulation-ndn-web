...
class PopsWithTrees:
	...
	def __insertChilds(self):
		for ipop in self.popnodes:
			self.G.node[ipop]['name'] = 'POP{}'.format(ipop)
	...
	def __genChildTree(self, pop):
		...
		auxchild1 = self.cn+1
		auxchild2 = self.cn+2
		self.G.add_node(auxrouter, ir=1, name='IR{}'.format(self.ir))
		self.G.add_node(auxchild1, cr=1, name='CR{}'.format(self.cr))
		self.G.add_node(auxchild2, cr=1, name='CR{}'.format(self.cr+1))
	...
	def SaveNS3(self, savefile):
		...
		for inode in sorted(self.G.node):
			arqout.write(NODEDEF.format(self.G.node[inode]['name']))
		arqout.write('\n# Links:\nlink\n')
		for ilb in sorted(self.G.edge):
			for ile in sorted(self.G.edge[ilb]):
				arqout.write(LINKDEF.format(self.G.node[ilb]['name'], self.G.node[ile]['name']))
		arqout.close()
		return

class PopsWithoutTreesVersioned:
